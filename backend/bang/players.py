...
class Player:
        ...
        if r.status_code == 200:
            ...
            if self.game:
                G.sio.emit(
                    "chat_message",
                    room=self.game.name,
                    data=f'_change_username|{self.name}|{res["username"]}',
                )
            self.name = res["username"]
            self.discord_id = res["id"]
            if self.is_admin():
                if self.game:
                    self.game.feature_flags()
                G.sio.emit(
                    "chat_message",
                    room=self.sid,
                    data={"color": "green", "text": "(you are admin)"},
                )
            if self.game:
    ...
    def set_role(self, role: r.Role):
        ...
        G.sio.emit(
            "role", room=self.sid, data=json.dumps(role, default=lambda o: o.__dict__)
        )
    ...
    def set_character(self, character: str):
        ...
        if self.character is None:
            try:
                ...
                self.character = next(
                    x
                    for x in self.available_characters
                    if x.name.split()[1] == character.split()[1]
                )
            if "high_noon" in self.game.expansions:
                # questo viene utilizzato per la carta nuova identità
                self.not_chosen_character = next(
                    x for x in self.available_characters if x.name != character
                )
            else:
                self.not_chosen_character = None
    ...
    def set_available_character(self, available):
        ...
        if not self.is_bot:
            G.sio.emit(
                "characters",
                room=self.sid,
                data=json.dumps(available, default=lambda o: o.__dict__),
            )
        else:
    ...
    def notify_card(self, player, card, message=""):
        try:
            ...
            pass
        mess = {"player": player.name, "card": card, "message": message}
    ...
    def notify_self(self):
        if any((True for c in self.equipment if isinstance(c, tvosc.Fantasma))):
            ...
            self.is_ghost
            and self.is_my_turn
            and not self.game.check_event(ceh.CittaFantasma)
        ):
        ...
        if self.pending_action == PendingAction.DRAW and self.game.check_event(
            ...
            and self.lives > 0
            and len(self.hand) == 0
            and (not self.is_my_turn or self.pending_action == PendingAction.PLAY)
        ):
        ...
        if self.lives <= 0 and self.max_lives > 0 and not self.is_dead:
            ...
            if self.character.check(self.game, chw.TerenKill):
                ...
                if not picked.check_suit(self.game, [cs.Suit.SPADES]):
                    ...
                    self.game.deck.draw(True, player=self)
            if (
                self.character.check(self.game, chars.SidKetchum)
                and len(self.hand) > 1
                and self.lives == 0
            ):
                if self.game.players[self.game.turn] != self:
                    self.game.players[
                        self.game.turn
                    ].pending_action = PendingAction.WAIT
        ...
        ser["can_gold_rush_discard"] = any(
            (
                p != self
                and any(
                    (e.number <= self.gold_nuggets + 1 for e in p.gold_rush_equipment)
                )
                for p in self.game.get_alive_players()
            )
        )
        if self.character:
            ser["gold_rush_discount"] = (
                1
                if self.character.check(self.game, grch.PrettyLuzena)
                and self.special_use_count < 1
                else 0
            )
        ...
        if self.lives <= 0 and self.max_lives > 0 and not self.is_dead:
            ...
            G.sio.emit(
                "self",
                room=self.sid,
                data=json.dumps(ser, default=lambda o: o.__dict__),
            )
            self.game.player_death(self)
        if (
            self.game and self.game.started
        ):  # falso quando un bot viene eliminato dalla partita
            G.sio.emit(
                "self_vis",
                room=self.sid,
                data=json.dumps(
        ...
        G.sio.emit(
            "self", room=self.sid, data=json.dumps(ser, default=lambda o: o.__dict__)
        )

    def bot_spin(self):
        while self.is_bot and self.game is not None and not self.game.shutting_down:
            G.sio.sleep(
                max(0.2, uniform(self.game.bot_speed / 2 - 0.1, self.game.bot_speed))
            )
            if self.lives > 0 or self.is_ghost:
                self.bot_logic()

    def bot_logic(self):
        if self.game is None or self.game.shutting_down:
            return
        if (
            self.pending_action is not None
            and self.pending_action != PendingAction.WAIT
        ):
        ...
        if self.pending_action == PendingAction.PICK:
            ...
            non_blocked_cards = [
                ...
                if (
                    not self.game.check_event(ceh.Manette)
                    or card.suit == self.committed_suit_manette
                )
                and (
                    not self.game.check_event(ce.IlGiudice) or not card.usable_next_turn
                )
            ]
            equippables = [
                c
                for c in non_blocked_cards
                if (c.is_equipment or c.usable_next_turn)
                and not isinstance(c, cs.Prigione)
                and not c.need_target
                and not any(
                    (
                        type(c) == type(x) and not (c.is_weapon and c.must_be_used)
            ...
            misc = [
                ...
                and (
                    isinstance(c, cs.WellsFargo)
                    or isinstance(c, cs.Indiani)
                    or isinstance(c, cs.Gatling)
                    or isinstance(c, cs.Diligenza)
                    or isinstance(c, cs.Emporio)
                    or (
                        (
                            isinstance(c, cs.Birra)
                            and self.lives < self.max_lives
                            or c.must_be_used
                        )
                    ...
                    or (
                        ...
                        and len(self.hand) > 1
                        and not (
                            isinstance(c, csd.Whisky) and self.lives == self.max_lives
                        )
                    )
                )
                and not (not c.can_be_used_now and self.game.check_event(ce.IlGiudice))
                and not c.is_equipment
            ]
            need_target = [
                c
                for c in non_blocked_cards
                ...
                and not (c.need_with and len(self.hand) < 2)
                and not (
                    type(c) == type(cs.Bang)
                    and (
                        self.game.check_event(ceh.Sermone)
                        or (
                            self.has_played_bang
                            and (
                                not any(
                                    (isinstance(c, cs.Volcanic) for c in self.equipment)
                                )
                                or self.game.check_event(ce.Lazo)
                            )
                        )
                    )
                )
                and not (
                    isinstance(c, cs.Prigione) and self.game.check_event(ce.IlGiudice)
                )
                or isinstance(c, cs.Duello)
                or isinstance(c, cs.CatBalou)
                or isinstance(c, csd.Pugno)
            ]
            green_cards = [
                c
                for c in self.equipment
                if not self.game.check_event(ce.Lazo)
                and not isinstance(c, cs.Mancato)
                    ...
                    if (
                        ...
                        if not card.need_target:
                            ...
                            others = [
                                ...
                                if _range >= p["dist"]
                                and not (
                                    isinstance(self.role, r.Vice) and p["is_sheriff"]
                                )
                            ]
                            if len(others) == 0:
                                continue
                            target = others[randrange(0, len(others))]
                            if target["is_sheriff"] and isinstance(
                                self.role, r.Renegade
                            ):
                                target = others[randrange(0, len(others))]
                            if self.play_card(
                                len(self.hand) + self.equipment.index(card),
            ...
            maxcards = (
                self.lives
                if not self.character.check(self.game, chd.SeanMallory)
                else 10
            )
            if maxcards == self.lives and any(
                (isinstance(c, grc.Cinturone) for c in self.gold_rush_equipment)
            ):
                maxcards = 8
            ...
            if not self.target_p:
                ...
                if len(target.hand) + len(target.equipment) == 0:
                    ...
                    try:
                        card_index = randrange(
                            0, len(target.hand) + len(target.equipment)
                        )
                        self.choose(card_index)
                        self.game.rpc_log.append(f"{self.name};choose;{card_index}")
                    except:
    ...
    def play_turn(self, can_play_vendetta=True, again=False):
        if ((self.lives == 0 or self.is_dead) and not self.is_ghost) or (
            self.is_ghost
            and any(isinstance(c, tvosc.Fantasma) for c in self.equipment)
            and self.game.check_event(ce.Lazo)
        ):
        ...
        if (
            self.game.check_event(ce.FratelliDiSangue)
            and self.lives > 1
            and not self.is_giving_life
            and sum(
                p != self and p.lives < p.max_lives
                for p in self.game.get_alive_players()
            )
        ):
            self.set_choose_action(
            ...
            (
                ...
                or isinstance(c, cs.Prigione)
                or isinstance(c, tvosc.SerpenteASonagli)
                for c in self.equipment
            )
        ):
            self.is_giving_life = False
            ...
            if isinstance(self.real_character, chd.VeraCuster):
                self.set_available_character(
                    [p.character for p in self.game.get_alive_players() if p != self]
                )
            else:
                self.pending_action = PendingAction.DRAW
        self.notify_self()
    ...
    def draw(self, pile):
        if (
            ...
            and self.pending_action == PendingAction.PLAY
            and pile == "event"
            and self.game.check_event(ce.Cecchino)
        ...
        if (
            ...
            self.game.deck.event_cards[0].on_clicked(self.game, self)
        elif (
            self.is_my_turn
            and self.pending_action == PendingAction.PLAY
            and pile == "event_wildwestshow"
        ):
            self.game.deck.event_cards_wildwestshow[0].on_clicked(self.game, self)
        if self.pending_action != PendingAction.DRAW:
            return
        if (
            pile == "event"
            and self.lives < self.max_lives
            and self.game.check_event(ce.LiquoreForte)
        ):
            self.lives += 1
            self.pending_action = PendingAction.PLAY
            self.notify_self()
            ...
            and type(pile) == str
            and pile != self.name
            and pile in self.game.players_map
                ...
                for p in self.game.get_alive_players():
                    if p != self and len(p.hand) > hsize:
                        G.sio.emit(
                            "card_drawn",
                            room=self.game.name,
                            data={"player": self.name, "pile": p.name},
                        )
                        self.hand.append(p.hand.pop(randint(0, len(p.hand) - 1)))
            num = (
                2
                if not self.character.check(self.game, chd.BillNoface)
                else self.max_lives - self.lives + 1
            )
            if self.character.check(self.game, chd.PixiePete):
                num += 1
            if self.character.check(self.game, tvosch.TucoFranziskaner) and not any(
                (True for c in self.equipment if not c.usable_next_turn)
            ):
                num += 2
            if any((isinstance(c, grc.Piccone) for c in self.gold_rush_equipment)):
                num += 1
            if self.game.check_event(ceh.IlTreno) or (
                self.is_ghost and self.game.check_event(ceh.CittaFantasma)
            ):
                num += 1
            elif self.game.check_event(ceh.Sete):
                num -= 1
            for i in range(num):
                if (
                    i == 0
                    and pile == "scrap"
                    and self.character.check(self.game, chars.PedroRamirez)
                ):
                    ...
                    and pile != self.name
                    and pile in self.game.players_map
                    and self.character.check(self.game, chars.JesseJones)
                    and len(self.game.get_player_named(pile).hand) > 0
                ):
                    self.hand.append(
                        self.game.get_player_named(pile).hand.pop(
                    ...
                    if self.character.check(
                        self.game, chars.BlackJack
                    ) or self.game.check_event(ce.LeggeDelWest):
                        for p in self.game.get_alive_players():
                            if p != self:
                                p.notify_card(
                                    self,
                                    card,
                                    "blackjack_special"
                                    if self.character.check(self.game, chars.BlackJack)
                                    else "foc.leggedelwest",
                                )
                        ...
                        if self.character.check(
                            ...
                            self.game, [cs.Suit.HEARTS, cs.Suit.DIAMONDS]
                        ):
                    ...
                    G.sio.emit(
                        "card_drawn",
                        room=self.game.name,
                        data={"player": self.name, "pile": pile},
                    )
                else:
                    self.game.deck.draw(player=self)
            self.manette()
            ...
            for i, card in enumerate(self.equipment):
                if i < len(self.equipment) and isinstance(card, cs.Dinamite):
                    while pickable_cards > 0:
                        ...
                        if (
                            picked.check_suit(self.game, [cs.Suit.SPADES])
                            and 2 <= picked.number <= 9
                            and pickable_cards == 0
                        ):
                            self.lives -= 3
                            if any(
                                (
                                    isinstance(c, grc.Talismano)
                                    for c in self.gold_rush_equipment
                                )
                            ):
                                self.gold_nuggets += 3
                            if self.character.check(self.game, grch.SimeonPicos):
                            ...
                            if any(
                                ...
                                self.game.deck.draw(player=self)
                            self.attacker = None
                            self.game.deck.scrap(
                                self.equipment.pop(i), True, player=self
                            )
                            G.sio.emit(
                                "chat_message",
                                room=self.game.name,
                            ...
                            self.heal_if_needed()
                            if (
                                self.character.check(self.game, chars.BartCassidy)
                                and self.lives > 0
                            ):
                                for i in range(3):
                                    self.game.deck.draw(True, player=self)
                            ...
                            self.game.next_player().equipment.append(
            ...
            for i, card in enumerate(self.equipment):
                if isinstance(card, cs.Prigione):
                    while pickable_cards > 0:
                        ...
                        if (
                            not picked.check_suit(self.game, [cs.Suit.HEARTS])
                            and pickable_cards == 0
                        ):
                            self.game.deck.scrap(
                                self.equipment.pop(i), True, player=self
    ...
    def get_playable_cards(self):
        ...
        for i, card in enumerate(self.hand):
            if (
                ...
                and not any((isinstance(c, cs.Volcanic) for c in self.equipment))
            ):
                continue
    ...
    def play_card(self, hand_index: int, against=None, _with=None):
        ...
        if (
            not self.is_my_turn
            or self.pending_action != PendingAction.PLAY
            or self.game.is_handling_death
        ):
            print("but cannot")
            return
        if not (
            0
            <= hand_index
            < len(self.hand) + len(self.equipment) + len(self.gold_rush_equipment)
        ):
            print("but the card index is out of range")
            return
        elif len(self.hand) + len(self.equipment) <= hand_index < len(self.hand) + len(
            self.equipment
        ) + len(self.gold_rush_equipment) and len(self.gold_rush_equipment):
            print("which is a gold rush black card")
            card: grc.ShopCard = self.gold_rush_equipment[
                hand_index - len(self.hand) - len(self.equipment)
            ]
        ...
        card: cs.Card = (
            ...
            else self.equipment.pop(hand_index - len(self.hand))
        )
        withCard: cs.Card = None
        if _with is not None:
            withCard = (
                self.hand.pop(_with) if hand_index > _with else self.hand.pop(_with - 1)
            )
        print(self.name, "is playing ", card, " against:", against, " with:", _with)
        did_play_card = False
        event_blocks_card = (
            (
                self.game.check_event(ce.IlGiudice)
                and (
                    card.is_equipment
                    or (card.usable_next_turn and not card.can_be_used_now)
                )
            )
            or (
                self.game.check_event(ce.Lazo)
                and card.usable_next_turn
                and card.can_be_used_now
            )
            or (
        ...
        if (
            not (
                against is not None
                and (
                    self.game.get_player_named(against).character.check(
                        self.game, chd.ApacheKid
                    )
                    or any(
                        (
                            isinstance(c, grc.Calumet)
                            for c in self.game.get_player_named(
                                against
                            ).gold_rush_equipment
                        )
                    )
                )
                and card.check_suit(self.game, [cs.Suit.DIAMONDS])
            )
            or (isinstance(card, grc.ShopCard) and card.kind == grc.ShopCardKind.BLACK)
            and not event_blocks_card
        ):
            if (
                against == self.name
                and not isinstance(card, csd.Tequila)
                and not isinstance(card, cs.Panico)
                and not isinstance(card, cs.CatBalou)
            ) or event_blocks_card:
                did_play_card = False
            else:
                did_play_card = card.play_card(self, against, withCard)
        if (
            not card.is_equipment
            and not card.usable_next_turn
            and not (
                isinstance(card, grc.ShopCard) and card.kind == grc.ShopCardKind.BLACK
            )
            or (
                event_blocks_card
                and not (card.usable_next_turn and card.can_be_used_now)
            )
        ):
            if did_play_card:
            ...
            (card.usable_next_turn and card.can_be_used_now)
            or (isinstance(card, grc.ShopCard) and card.kind == grc.ShopCardKind.BLACK)
            or event_blocks_card
        ):
            if did_play_card:
        ...
        if not self.game.is_replay:
            Metrics.send_metric(
                ...
                points=[1],
                tags=[
                    f"success:{did_play_card}",
                    f"card:{card.name}",
                    f"bot:{self.is_bot}",
                    f'exp:{card.expansion if "expansion" in card.__dict__ else "vanilla"}',
                ],
            )
    ...
    def choose(self, card_index):
        ...
        if self.target_p and self.target_p != "":  # panico, cat balou, rissa
            ...
            if target.name == self.name:
                card = self.equipment.pop(
                    card_index
                    if card_index < len(target.hand)
                    else card_index - len(target.hand)
                )
            ...
            if self.choose_action == "steal":
                G.sio.emit(
                    "card_drawn",
                    room=self.game.name,
                    data={"player": self.name, "pile": target.name},
                )
                card.reset_card()
                if (
                    card.name != "Fantasma" or self.name != target.name
                ):  # se si uccide facendo panico su fantasma la carta non gli viene messa in mano
                    self.hand.append(card)
            else:
            ...
            G.sio.emit(
                ...
                data=f"_play_card_against|{self.name}|Ricercato|{player.name}",
            )
            ...
            if self.scrapped_cards == 2:
                ...
                if self.game.players[self.game.turn] != self:
                    self.game.players[
                        self.game.turn
                    ].pending_action = PendingAction.PLAY
                    self.game.players[self.game.turn].notify_self()
            self.notify_self()
            ...
            G.sio.emit(
                "chat_message",
                room=self.game.name,
                data=f'_play_card_for|{self.name}|{"Bicchierino"}|{player.name}',
            )
            ...
            if card_index == 0:
                self.gold_nuggets += 1
                G.sio.emit(
                    "chat_message", room=self.game.name, data=f"_get_nugget|{self.name}"
                )
            else:
            ...
            G.sio.emit(
                "chat_message",
                room=self.game.name,
                data=f'_play_card|{self.name}|{"Bottiglia"}',
            )
            ...
            if card_index == len(self.available_cards) - 1:
                ...
                self.available_cards = [
                    c
                    for c in player.gold_rush_equipment
                    if c.number + 1 <= self.gold_nuggets
                ]
            ...
            if card_index == len(self.available_cards) - 1:
                ...
                G.sio.emit(
                    "chat_message",
                    room=self.game.name,
                    data=f"_gold_rush_pay_discard|{self.name}|{player.name}|{self.available_cards[card_index].name}",
                )
                player.notify_self()
            self.pending_action = PendingAction.PLAY
            self.notify_self()
            ...
            if card_index <= len(self.available_cards):
                ...
                G.sio.emit(
                    "chat_message",
                    room=player.game.name,
                    data=f"_play_card_against|{self.name}|Fantasma|{player.name}",
                )
            self.pending_action = PendingAction.PLAY
            self.notify_self()
            ...
            if card_index <= len(self.available_cards):
                ...
                G.sio.emit(
                    "chat_message",
                    room=player.game.name,
                    data=f"_play_card_against|{self.name}|Sventagliata|{og}",
                )
                G.sio.emit(
                    "chat_message",
                    room=player.game.name,
            ...
            if self.choose_text.split(";")[1] == "1":
                ...
                self.choose_text = "choose_from_poker;1"
            self.notify_self()
            ...
            if card_index < len(self.hand):
                self.available_cards = [
                    ...
                    for p in self.game.get_alive_players()
                    if p.name != self.name and len(p.hand) > 0
                ]
            ...
            if card_index < len(self.available_cards):
                ...
                other_player = self.game.get_player_named(
                    self.available_cards[card_index]["name"]
                )
                for i in range(min(2, len(other_player.hand))):
                    self.hand.append(
                        other_player.hand.pop(randint(0, len(other_player.hand) - 1))
                    )
            ...
            self.game.check_event(ceh.NuovaIdentita)
            and self.choose_text == "choose_nuova_identita"
        ):
            if card_index == 1:  # the other character
                ...
                G.sio.emit(
                    "chat_message",
                    room=self.game.name,
                    data=f"_choose_character|{self.name}|{self.character.name}",
                )
            self.play_turn(again=True)
        elif (
            self.game.check_event(ceh.Manette) and self.choose_text == "choose_manette"
        ):
            self.committed_suit_manette = cs.Suit(card_index)
            G.sio.emit(
                "chat_message",
                room=self.game.name,
                data=f'_choose_manette|{self.name}|{"♦♣♥♠"[card_index]}',
            )
            self.pending_action = PendingAction.PLAY
            self.notify_self()
            ...
            try:
                ...
                G.sio.emit(
                    "chat_message",
                    room=self.game.name,
                    data=f"_fratelli_sangue|{self.name}|{player.name}",
                )
            except:
                pass
            self.play_turn(again=True)
            ...
            try:
                if self.available_cards[card_index]["name"] != "":
                    for _ in range(2):
                        card = next(
                            c for c in self.hand if c.name == cs.Bang(0, 0).name
                        )
                        self.hand.remove(card)
                        self.game.deck.scrap(card, True)
                    self.pending_action = PendingAction.PLAY
                    self.game.attack(
                        self, self.available_cards[card_index]["name"], double=True
                    )
            except:
                self.pending_action = PendingAction.PLAY
            self.is_using_checchino = False
            ...
            if self.using_rimbalzo == 1 and "name" in self.available_cards[card_index]:
                ...
                self.available_cards = self.game.get_player_named(
                    self.available_cards[card_index]["name"]
                ).equipment
                ...
                self.using_rimbalzo == 2
                and "name" in self.available_cards[card_index].__dict__
            ):
            ...
            card = next(
                c for c in self.equipment if c == self.available_cards[card_index]
            )
            ...
            G.sio.emit(
                "chat_message",
                room=self.game.name,
                data=f"_guess|{self.name}|{self.available_cards[card_index]['icon']}",
            )
            self.available_cards = []
        ...
        # specifico per personaggio
            ...
            if self.game.check_event(ceh.Sete):
                pickable_stop += 1
            if self.game.check_event(ceh.IlTreno) or any(
                (isinstance(c, grc.Piccone) for c in self.gold_rush_equipment)
            ):
                pickable_stop -= 1
            if len(self.available_cards) == pickable_stop:
                if (
                    len(self.available_cards) > 0
                ):  # la carta non scelta la rimettiamo in cima al mazzo
                ...
                if self.game.check_event(ceh.IlTreno) and any(
                    (isinstance(c, grc.Piccone) for c in self.gold_rush_equipment)
                ):
                    self.game.deck.draw(player=self)
                self.is_drawing = False
        ...
        # specifico per personaggio
            ...
            if not self.game.check_event(ceh.Sete):
                self.hand.append(
                    self.available_cards.pop(card_index)
                )  # prendo la carta scelta
            else:
                self.game.deck.scrap(
                    self.available_cards.pop(0), True
                )  # non pesco carte
        ...
        # specifico per personaggio
            ...
            G.sio.emit(
                "card_drawn",
                room=self.game.name,
                data={"player": self.name, "pile": self.pat_target},
            )
    ...
    def barrel_pick(self):
        ...
        if any((isinstance(c, grc.FerroDiCavallo) for c in self.gold_rush_equipment)):
            pickable_cards += 1
        if any(
            (isinstance(c, cs.Barile) for c in self.equipment)
        ) and self.character.check(self.game, chars.Jourdonnais):
            pickable_cards = 2
        while pickable_cards > 0:
            pickable_cards -= 1
            picked: cs.Card = self.game.deck.pick_and_scrap()
        ...
        if (
            ...
            and not any(
                (
                    c.can_be_used_now and isinstance(c, cs.Mancato)
                    for c in self.equipment
                )
            )
        ):
    ...
    def get_banged(
        ...
        if (
            ...
            G.sio.emit(
                "chat_message",
                room=self.game.name,
                data=f"_flipped|{attacker.name}|{picked.name}|{picked.num_suit()}",
            )
        ...
        if (
            not self.game.is_competitive
            and not any((isinstance(c, cs.Barile) for c in self.equipment))
            and not self.character.check(self.game, chars.Jourdonnais)
            and not any(
                (
                    (isinstance(c, cs.Mancato) and c.can_be_used_now)
                    or (
                        self.character.check(self.game, chars.CalamityJanet)
                        and isinstance(c, cs.Bang)
                    )
            ...
            if (
                (
                    not self.game.check_event(ce.Lazo)
                    and any((isinstance(c, cs.Barile) for c in self.equipment))
                )
                and not (
                    self.game.players[self.game.turn].character.check(
                        self.game, chd.BelleStar
                    )
                    and isinstance(attacker, Player)
                )
            ) or self.character.check(
                self.game, chars.Jourdonnais
            ):  # se ho un barile e non c'è lazo e non mi sta attaccando Belle Star o se sono Jourdonnais
                print("has barrel")
                self.pending_action = PendingAction.PICK
                if not no_dmg:
                ...
                if (
                    self.attacker
                    and self.attacker in self.game.get_alive_players()
                    and attacker.character.check(self.game, chd.BelleStar)
                    or self.game.check_event(ce.Lazo)
                ):
                    self.expected_response = (
                        self.game.deck.mancato_cards_not_green_or_blue.copy()
                    )
                if (
                    self.character.check(self.game, chars.CalamityJanet)
    ...
    def get_indians(self, attacker):
        ...
        if self.character.check(self.game, chd.ApacheKid) or any(
        ...
        if (
            ...
            if (
                self.character.check(self.game, chars.CalamityJanet)
                or self.can_escape(with_mancato=True)
    ...
    def heal_if_needed(self):
        while (
            self.lives <= 0
            and len(self.game.get_alive_players()) > 2
            and any((isinstance(c, cs.Birra) for c in self.hand))
            and not self.game.check_event(ceh.IlReverendo)
        ):
            for i in range(len(self.hand)):
                if isinstance(self.hand[i], cs.Birra):
                    if (
                        self.character.check(self.game, chd.MollyStark)
                        and not self.is_my_turn
                    ):
                        self.game.deck.draw(True, player=self)
                    self.lives += (
                        1 if not self.character.check(self.game, chd.TequilaJoe) else 2
                    )
    ...
    def take_damage_response(self):
        ...
        if self.lives > 0:
            if self.character.check(self.game, chars.BartCassidy):
                ...
                self.character.check(self.game, chars.ElGringo)
                and self.attacker
                and self.attacker in self.game.get_alive_players()
                and len(self.attacker.hand) > 0
            ):
                self.hand.append(
                    self.attacker.hand.pop(randrange(0, len(self.attacker.hand)))
                )
                self.hand[-1].reset_card()
                G.sio.emit(
                    "card_drawn",
                    room=self.game.name,
                    data={"player": self.name, "pile": self.attacker.name},
                )
                G.sio.emit(
                    "chat_message",
                    room=self.game.name,
        ...
        if isinstance(self.attacker, Player) and not self.game.check_event(ce.Lazo):
            if any((isinstance(c, tvosc.Taglia) for c in self.equipment)):
                ...
                self.attacker.notify_self()
            if len(self.hand) > 0 and any(
                (isinstance(cd, tvosc.Shotgun) for cd in self.attacker.equipment)
            ):
                ...
                G.sio.emit(
                    "chat_message",
                    room=self.game.name,
                    data=f"_shotgun_scrap|{self.name}|{c.name}",
                )
    ...
    def take_no_damage_response(self):
        if (
            self.dmg_card_index is not None
            and self.dmg_card_index != -1
            and self.game.check_event(ce.Rimbalzo)
        ):
    ...
    def respond(self, hand_index):
        if self.pending_action != PendingAction.RESPOND:
            return
        self.pending_action = PendingAction.WAIT
        if (
            hand_index != -1
            and hand_index < (len(self.hand) + len(self.equipment))
            and (
                (
                    (
                        hand_index < len(self.hand)
                        and self.hand[hand_index].name in self.expected_response
                    )
                    or self.character.check(self.game, chd.ElenaFuente)
                )
                or (
                    0 <= hand_index - len(self.hand) < len(self.equipment)
                    and self.equipment[hand_index - len(self.hand)].name
                    in self.expected_response
                )
            )
        ):
            card = (
            ...
            if (
                self.character.check(self.game, chd.MollyStark)
                and 0 <= hand_index <= len(self.hand)
                and not self.is_my_turn
                and self.event_type != "duel"
            ):
                if (
                    hasattr(self.attacker, "character")
                    and self.attacker.character.check(self.game, chars.SlabTheKiller)
                    and isinstance(card, cs.Mancato)
                ):
            ...
            G.sio.emit(
                "chat_message",
                room=self.game.name,
                data=f"_respond|{self.name}|{card.name}",
            )
            self.game.deck.scrap(card, True, player=self)
            self.notify_self()
            self.mancato_needed -= 1
            ...
            if self.mancato_needed <= 0:
                if self.event_type == "duel":
                    if isinstance(card, tvosc.Fuga) or (
                        isinstance(card, cs.Mancato)
                        and self.character.check(self.game, tvosch.MickDefender)
                    ):
                        self.game.responders_did_respond_resume_turn(did_lose=False)
                    else:
            ...
            if self.character.check(self.game, chd.MollyStark) and not self.is_my_turn:
                ...
                self.attacker
                and self.attacker in self.game.get_alive_players()
                and self.attacker.character.check(self.game, chd.MollyStark)
                and self.is_my_turn
            ):
    ...
    def get_sight(self, countWeapon=True):  # come vedo io gli altri
        ...
        return (
            ...
            + aim
            + (
                self.character.sight_mod
                if not self.game.check_event(ceh.Sbornia)
                else 0
            )
        )
    ...
    def get_visibility(self):  # come mi vedono gli altri
        if (
            not self.character
            or not self.game
            or not self.game.players[self.game.turn].character
        ):
            return 0
        covers = 0
        ch_vis_mod = (
            self.character.visibility_mod
            if not self.game.check_event(ceh.Sbornia)
            else 0
        )
        if self.game.check_event(ce.Lazo) or self.game.players[
            self.game.turn
        ].character.check(self.game, chd.BelleStar):
            return ch_vis_mod
        for card in self.equipment:
            covers += card.vis_mod
    ...
    def scrap(self, card_index):
        if len(self.hand) == 0 or len(self.hand) <= card_index:
            return self.notify_self()
        if self.is_my_turn or self.character.check(self.game, chars.SidKetchum):
            ...
            if (
                self.character.check(self.game, chars.SidKetchum)
                and self.scrapped_cards == 2
            ):
                self.scrapped_cards = 0
                self.lives = min(self.lives + 1, self.max_lives)
            elif (
                self.character.check(self.game, chd.JoseDelgado)
                and card.is_equipment
                and self.special_use_count < 2
            ):
                self.game.deck.draw(True, player=self)
                self.game.deck.draw(True, player=self)
    ...
    def buy_gold_rush_card(self, index):
        print(
            f"{self.name} wants to buy gr-card index {index} in room {self.game.name}"
        )
        card: cs.Card = self.game.deck.shop_cards[index]
        discount = 0
        if (
            self.character.check(self.game, grch.PrettyLuzena)
            and self.special_use_count < 1
        ):
            discount = 1
        if (
            self.pending_action == PendingAction.PLAY
            and self.gold_nuggets >= card.number - discount
        ):
    ...
    def check_can_end_turn(self):
        ...
        if self.game.check_event(ce.LeggeDelWest) and len(must_be_used_cards) > 0:
            ...
            print(
                self.has_played_bang
                and not (
                    any((isinstance(c, cs.Volcanic) for c in self.equipment))
                    and type(card) == type(cs.Bang)
                )
            )
            if (
                card.suit == cs.Suit.DIAMONDS
                and card.need_target
                and not any(
                    (
                        (
                            not p.character.check(self.game, chd.ApacheKid)
                            and not any(
                                (
                                    isinstance(c, grc.Calumet)
                                    for c in p.gold_rush_equipment
                                )
                            )
                        )
                        for p in self.game.get_alive_players()
                ...
                (
                    ...
                    or (
                        isinstance(card, cs.Mancato)
                        and self.character.check(self.game, chars.CalamityJanet)
                    )
                )
                and self.has_played_bang
                and not any((isinstance(c, cs.Volcanic) for c in self.equipment))
                or not any(
                    (
                        self.get_sight() >= p["dist"]
                        for p in self.game.get_visible_players(self)
                    )
                )
            ):
                return True
            elif isinstance(card, cs.Mancato) or (
                card.need_with and len(self.hand) < 2
            ):
                return True
            elif (
                isinstance(card, cs.Panico)
                and not any(
                    (
                        self.get_sight(False) >= p["dist"]
                        for p in self.game.get_visible_players(self)
                    )
                )
                ...
                return True
            elif isinstance(card, csd.Pugno) and not any(
                (
                    self.get_sight(False) >= p["dist"]
                    for p in self.game.get_visible_players(self)
                )
                ...
                return True
            elif isinstance(card, cs.Prigione) and not any(
                (not p["is_sheriff"] for p in self.game.get_visible_players(self))
            ):
                return True
            elif not card.is_weapon and any(
                (c.name == card.name for c in self.equipment)
            ):
                return True
            return False
        return True
    ...
    def end_turn(self, forced=False):
        ...
        if not self.is_my_turn and not forced:
            return
        maxcards = (
            self.lives if not self.character.check(self.game, chd.SeanMallory) else 10
        )
        if maxcards == self.lives and any(
            (isinstance(c, grc.Cinturone) for c in self.gold_rush_equipment)
        ):
        ...
        if len(self.hand) > maxcards and not forced:
            ...
            for i in range(len(self.equipment)):
                if (
                    self.equipment[i].usable_next_turn
            ...
            if (
                not forced
                and self.game.check_event(ce.Vendetta)
                and self.can_play_vendetta
            ):
                picked: cs.Card = self.game.deck.pick_and_scrap()
                G.sio.emit(
                    "chat_message",
            ...
            if (
                not forced
                and self.character.check(self.game, grch.DonBell)
                and self.can_play_again_don_bell
            ):
                picked: cs.Card = self.game.deck.pick_and_scrap()
                G.sio.emit(
                    "chat_message",
            ...
            if (
                self.is_dead
                and self.is_ghost
                and self.game.check_event(ceh.CittaFantasma)
                and not any(
                    (True for c in self.equipment if isinstance(c, tvosc.Fantasma))
                )
            ) or (
                self.is_dead
                and forced
